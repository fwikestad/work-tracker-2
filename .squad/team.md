# Squad Team

> work-tracker-2

## Coordinator

| Name | Role | Notes |
|------|------|-------|
| Squad | Coordinator | Routes work, enforces handoffs and reviewer gates. |

## Members

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Han | Lead | [charter](.squad/agents/han/charter.md) | 🟢 Active |
| Leia | Frontend Dev | [charter](.squad/agents/leia/charter.md) | 🟢 Active |
| Chewie | Backend Dev | [charter](.squad/agents/chewie/charter.md) | 🟢 Active |
| Wedge | Tester | [charter](.squad/agents/wedge/charter.md) | 🟢 Active |
| Mon Mothma | Technical Writer | [charter](.squad/agents/mothma/charter.md) | 🟢 Active |
| Ackbar | Security Expert | [charter](.squad/agents/ackbar/charter.md) | 🟢 Active |
| Lando | DevOps Expert | [charter](.squad/agents/lando/charter.md) | 🟢 Active |
| Scribe | Session Logger | [charter](.squad/agents/scribe/charter.md) | 🟢 Active |
| Ralph | Work Monitor | — | 🔄 Monitor |
| Fredrik | 👤 Human — Approvals | — | 🟢 Active |

## Project Context

- **Project:** work-tracker-2 — Native desktop time tracker for consultants
- **User:** Fredrik Kristiansen Wikestad
- **Stack:** TBD (tech-agnostic; native desktop, local-only, embedded DB)
- **Description:** Personal time tracker — multiple customers/work orders per day, fast context switching, active timer visibility, daily summaries, CSV export
- **Universe:** Star Wars (OT)
- **Created:** 2026-04-11
